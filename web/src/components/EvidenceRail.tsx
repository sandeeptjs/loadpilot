...
/* The backend often repeats the same sentence across summary, ai_summary and evidence.
   Collapse the repeats so the card reads as an argument rather than an echo. */
function Finding({ investigation }: { investigation: Investigation }) {
  ...
  const detail = investigation.ai_summary ?? investigation.summary
  const body = [detail, ...investigation.evidence, ...(investigation.limitations ?? [])]
    .filter((item, index, all) => item && item !== headline && all.indexOf(item) === index)
  return <section className="finding"><h2>Analysis</h2>
    <strong>{headline}</strong>
    <span>Evidence assessment: {Math.round(investigation.confidence * 100)} / 100, heuristic</span>
    {body.map((item) => <p key={item}>{item}</p>)}
    {investigation.recommended_next_experiment && <p className="next"><b>Next:</b> {investigation.recommended_next_experiment}</p>}
  </section>
}
...
export function EvidenceRail({ run, plan, investigation }: { run: Run; plan: Plan; investigation?: Investigation }) {
  ...
  return <aside className="evidence-rail">
    ...
    {investigation && <Finding investigation={investigation} />}
    <section><h2>Thresholds</h2>{plan.thresholds.length > 0 ? <ul className="threshold-list">{plan.thresholds.map((threshold) => <li key={threshold.metric}><span>{threshold.metric}</span><strong>{threshold.expression}</strong></li>)}</ul> : <div className="rail-empty"><strong>No thresholds declared</strong><span>This plan measures behaviour without asserting pass or fail conditions.</span></div>}</section>
