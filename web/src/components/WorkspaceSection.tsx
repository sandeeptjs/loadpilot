...
export function WorkspaceSection({ section, runs, selected, onSelect }: { section: Section; runs: Run[]; selected?: string; onSelect: (id: string) => void }) {
  const [events, setEvents] = useState<Array<{ id: string; action: string; timestamp: string; reason: string; related_test_id?: string }>>([])
  useEffect(() => { if (section === 'Audit') void fetch('/api/audit?limit=200').then(r => r.json()).then(setEvents).catch(() => setEvents([])) }, [section])
  const meta = copy[section]
  const completed = runs.filter((run) => run.state === 'COMPLETED' && run.slo_passed !== false)
  const incidents = runs.filter((run) => ['FAILED', 'TIMED_OUT', 'CANCELED'].includes(run.state) || run.slo_passed === false)
  ...
  return <div className="section-page">
    <header className="section-heading"><div><span className="section-icon">{icon}</span><h1>{meta.title}</h1><p>{meta.description}</p></div><span className="section-count">{section === 'Audit' ? events.length : section === 'Baselines' ? completed.length : section === 'Incidents' ? incidents.length : runs.length} records</span></header>
    ...
    {section === 'Incidents' && <div className="section-cards">{incidents.map((run) => <article className="section-card incident" key={run.id}><span>NEEDS REVIEW · {run.state.replaceAll('_', ' ')}</span><strong>{run.id.slice(0, 8)}</strong><p>{run.error ?? (run.slo_passed === false ? 'Performance limits breached; the evidence needs review.' : `Run ended as ${run.state.replaceAll('_', ' ').toLowerCase()}.`)}</p><button className="button secondary" onClick={() => onSelect(run.id)}>Inspect run</button></article>)}{!incidents.length && <Empty label="No failed, timed-out, or canceled runs. Everything is clear." />}</div>}
    {section === 'Audit' && <div className="audit-list">{events.map(event => <article key={event.id}><ScrollText size={16} /><div><strong>{event.action}</strong><p>{new Date(event.timestamp).toLocaleString()} · {event.reason}</p></div>{event.related_test_id && <button className="button secondary" onClick={() => onSelect(event.related_test_id!)}>Open run</button>}</article>)}{!events.length && <Empty label="No audit events recorded yet." />}</div>}
  </div>
}
